    ...
    <>
      ...
      <div className="bg-gray-bg border-b border-gray-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="text-sm text-gray-muted flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-navy font-medium">Costos de Fianzas</span>
          </nav>
        </div>
      </div>
      ...
      <section className="bg-gradient-to-br from-navy to-navy/90 py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-white/80 text-lg max-w-3xl mx-auto">
            El costo generalmente se expresa como un porcentaje del monto total afianzado, conocido como prima. Esta prima varía entre el <span className="text-brand-light font-bold">1% y el 3%</span> del valor del contrato.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-gray-bg rounded-2xl p-6 md:p-8 border border-gray-border">
            <p className="text-gray-text text-lg leading-relaxed">
              Por ejemplo, si tu contrato con gobierno es por <span className="font-bold text-navy">$5,000,000 MXN</span> y la prima es del 1.5%, el costo de tu fianza sería aproximadamente <span className="font-bold text-brand">$75,000 MXN</span>. Este monto se paga una sola vez al momento de la emisión de la póliza.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-[var(--font-heading)] text-2xl md:text-3xl font-bold text-navy mb-8">
            Factores que afectan el costo
          </h2>
          <div className="space-y-4">
            {FACTORS.map((factor, i) => (
              <div key={i} className="flex items-start gap-4 bg-gray-bg rounded-xl p-5 border border-gray-border">
                <span className="w-8 h-8 bg-brand/10 rounded-lg flex items-center justify-center flex-shrink-0 text-brand font-bold text-sm">
                  {i + 1}
                </span>
                <div>
                  <h3 className="font-[var(--font-heading)] font-bold text-navy">{factor.title}</h3>
                  <p className="text-gray-muted text-sm mt-1">{factor.desc}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            <table className="w-full bg-white rounded-2xl shadow-md overflow-hidden">
              ...
              <tbody>
                {COST_TABLE.map((row, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-bg"}>
                    <td className="px-6 py-4 text-sm font-medium text-navy">{row.tipo}</td>
                    <td className="px-6 py-4 text-sm text-brand font-bold">{row.prima}</td>
                    <td className="px-6 py-4 text-sm text-gray-text">{row.ejemplo}</td>
                    <td className="px-6 py-4 text-sm text-gray-muted">{row.vigencia}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {DOCUMENTS.map((doc, i) => (
              <div key={i} className="flex items-start gap-3 bg-gray-bg rounded-xl p-4 border border-gray-border">
                ...
                <span className="text-gray-text text-sm">{doc}</span>
              </div>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="py-12 md:py-16 bg-gradient-to-r from-navy to-navy/90">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 bg-brand hover:bg-brand-dark text-white font-bold px-8 py-4 rounded-full transition-all text-lg"
            >
